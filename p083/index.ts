...
const matrix_string: string = fs.readFileSync('p083_matrix.txt').toString()
...
class node {
    ...
    down: node | undefined = undefined
    up: node | undefined = undefined
...
const start_node = getNode(matrix, 0, 0)
for (let i = 0; i < 80; i++) {
    for (let j = 0; j < 80; j++) {
        getNode(matrix, i, j)
    }
